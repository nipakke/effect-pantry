...
```
ERROR: Agent not found: development/devops-specialist
```
...
```bash
# Check file exists
ls -la .opencode/agent/subagents/development/devops-specialist.md

# Check registry
cat registry.json | jq '.components.agents[] | select(.id == "devops-specialist")'
```
