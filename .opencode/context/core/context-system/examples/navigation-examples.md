...
**Backend** → TypeScript, Node.js, infrastructure
...
- ✅ Organized by concern (backend, data)
...
Frontend Code (development/backend/):
│ ├── hooks-patterns.md
...
| Task | Path |
|------|------|
| **TypeScript patterns** | `backend/hooks-patterns.md` |
| **Effect-TS** | `backend/effect/query-patterns.md` |
| **Animations** | `../../` |
| **Styling** | `../../` |
...
**TypeScript** → `backend/`
**TypeScript** → `frontend/`
**Effect-TS** → `backend/typescript/`

## By Concern

**Code patterns** → `development/backend/`
**Visual design** → ``
```
...
- ✅ Task-focused (Backend development)
...
### TypeScript + Effect-TS + PostgreSQL
```
...
### T3 Stack (Effect-TS, tRPC, Prisma, Drizzle)
```
...
| Layer | Navigate To |
|-------|-------------|
| **Frontend** | `backend-navigation.md` |
| **Backend** | `backend-navigation.md` |
...
1. `backend/component-architecture.md` (structure)
2. `backend/hooks-patterns.md` (logic)
3. `` (styling)
```
...
Effect Schema allow you to use state and lifecycle features in functional components. The most common hooks are:
