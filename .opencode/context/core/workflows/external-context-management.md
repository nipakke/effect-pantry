...
├── /
│   ├── effect-layer-integration.md        # Fetched: " + Effect-TS setup"
│   ├── drizzle-adapter.md           # Fetched: "Drizzle adapter for "
│   └── session-management.md        # Fetched: "Session handling"
├── effect-ts/
│   ├── app-router-setup.md          # Fetched: "Layer system basics"
│   ├── server-actions.md            # Fetched: "Effect services patterns"
│   └── middleware.md                # Fetched: "Middleware configuration"
└── effect-ts/
    ├── server-components.md         # Fetched: "Effect-TS + Layer system"
...
- **Package name** (directory): Exact npm package name (kebab-case)
  - ✅ `drizzle-orm`, ``, `effect-ts`, `@effect/schema`
  - ❌ `drizzle`, `effect`, `effect-ts`

- **File name** (topic): Kebab-case description of the topic
  - ✅ `modular-schemas.md`, `effect-layer-integration.md`, `server-components.md`
  - ❌ `modular schemas.md`, `EffectTS Integration.md`, `ServerComponents.md`
...
```json
{
  ...
  "packages": {
    ...
    "": {
      "files": ["effect-layer-integration.md", "drizzle-adapter.md", "session-management.md"],
      "last_updated": "2026-01-28T14:25:10Z",
      "source": "Context7 API",
      "official_docs": "https://.com"
    },
    "effect-ts": {
      "files": ["app-router-setup.md", "server-actions.md", "middleware.md"],
      "last_updated": "2026-01-28T14:20:05Z",
      "source": "Context7 API",
      "official_docs": "https://effect.org"
    }
  }
}
```
...
Main Agent (e.g., OpenAgent)
  ↓
  Detects: "User is asking about Drizzle +  + Effect-TS"
  ↓
...
ExternalScout
  ↓
  1. Detects libraries: Drizzle, , Effect-TS
  ↓
...
Main Agent
  ...
  Lists files:
    ...
    - .tmp/external-context//effect-layer-integration.md
    - .tmp/external-context/effect-ts/app-router-setup.md
  ↓
...
- `.tmp/external-context//effect-layer-integration.md` — Effect-TS Layer integration guide
- `.tmp/external-context//drizzle-adapter.md` — Drizzle adapter setup

### Effect-TS

- `.tmp/external-context/effect-ts/app-router-setup.md` — Layer system basics
- `.tmp/external-context/effect-ts/server-actions.md` — Effect services patterns
...
### Example 1: Drizzle +  Integration

**Main Agent Flow**:

```
1. User asks: "Set up Drizzle +  in Effect-TS"
2. Main agent calls ExternalScout
3. ExternalScout fetches:
   - drizzle-orm/modular-schemas.md
   - drizzle-orm/postgresql-setup.md
   - /effect-layer-integration.md
   - /drizzle-adapter.md
   - effect-ts/app-router-setup.md
...
- `.tmp/external-context//effect-layer-integration.md`
- `.tmp/external-context//drizzle-adapter.md`
...
### Example 2: Effect-TS + Layer system

**Main Agent Flow**:

```
1. User asks: "How do I use Effect-TS with Effect-TS Layer system?"
2. Main agent calls ExternalScout
3. ExternalScout fetches:
   - effect-ts/server-components.md
   - effect-ts/prefetching.md
   - effect-ts/server-components.md
