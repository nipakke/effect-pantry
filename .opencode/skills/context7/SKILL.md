...
- `libraryName` (required): The library name to search for (e.g., "effect", "drizzle", "zod", "vitest")
- `query` (required): A description of the topic for relevance ranking

**Response fields:**

- `id`: Library identifier for the context endpoint (e.g., `<EFFECT_ID>`)
...
### Effect-TS Schema documentation

```bash
# Find Effect-TS library ID
curl -s "https://context7.com/api/v2/libs/search?libraryName=effect&query=Schema" | jq '.results[0].id'
# Returns: library ID for Effect-TS

# Fetch useState documentation
curl -s "https://context7.com/api/v2/context?libraryId=<EFFECT_ID>&query=Schema+struct&type=txt"
```
...
curl -s "https://context7.com/api/v2/libs/search?libraryName=drizzle&query=schema" | jq '.results[0].id'

# Fetch app router documentation
curl -s "https://context7.com/api/v2/context?libraryId=<DRIZZLE_ID>&query=schema+migrations&type=txt"
```
