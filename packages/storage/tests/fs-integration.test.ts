...
const createTempDir = () => mkdtemp(join(tmpdir(), 'effect-pantry-storage-test-'));
...
const runUpload = (svc: Context.Tag.Service<typeof Storage>, key: string, body: string) =>
  svc.upload(key, body).pipe(Effect.flatMap(({ result }) => result));

// ── Layer factory ─────────────────────────────────────────────────────

const makeFsLayer = (root: string) =>
  layer.pipe(Layer.provide(Layer.succeed(StorageAdapter, fs({ root }))));
...
it.layer(Layer.empty)('Fs adapter', (it) => {
  it.scoped.skipIf(!isIntegration)('upload → download roundtrip via filesystem', () =>
    Effect.gen(function* () {
  ...
  it.scoped.skipIf(!isIntegration)('exists returns false for missing key via filesystem', () =>
    Effect.gen(function* () {
      const tmpDir = yield* Effect.promise(() => createTempDir());
      yield* Effect.addFinalizer(() => Effect.promise(() => removeDir(tmpDir)));
