...
const program = Effect.gen(function* () {
  ...
  const { result: cancelResult, progress: cancelProgress } =
    yield* s.upload("huge-file.bin", hugeFile)
  ...
  for (const { progress: p } of uploads) {
    yield* Stream.runForEach(p, (ev) =>
      Effect.log(`Upload: ${ev.loaded}/${ev.total}`),
    ).pipe(Effect.forkScoped)
  }
